<template>
  <v-container fluid class="pa-6 page-container">
              ...
              <v-col v-for="slot in agendaDoDia" :key="slot.timestamp" cols="12" sm="6" md="4" lg="3">
                ...
                >
                  <v-card-text class="pa-3 text-center">
                    ...
                    <div 
                      class="text-caption truncate-text" 
    ...
    <div v-if="showDebug" class="debug-info">
      <p>Typebot ID: {{ typebotId }}</p>
      <p>Show Chat Panel: {{ showChatPanel }}</p>
      <p>Display: {{ $vuetify.display.name }}</p>
    </div>
  </v-container>
</template>
...
.typebot-wrapper { 
  height: calc(600px - 64px); /* 600px da altura do card - 64px do header */
  overflow: hidden;
}
...
const salvarAgendamento = async () => {
  if (!nomeCliente.value || !servicoSelecionado.value) {
    return alert('Nome e serviço são obrigatórios.');
  }
