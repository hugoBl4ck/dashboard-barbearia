              ...
              <v-col v-for="slot in agendaDoDia" :key="slot.timestamp" cols="12" sm="6" md="4" lg="3">
                <v-card class="slot-card" :variant="slot.tipo === 'livre' ? 'outlined' : 'flat'" :color="getSlotColor(slot)" @click="handleItemClick(slot)" :disabled="slot.tipo === 'passado' || slot.tipo === 'ocupado-meio'">
                  <v-card-text class="pa-3 text-center">
                    ...
                    <div class="text-caption truncate-text" v-if="slot.tipo === 'agendamento'">{{ slot.detalhes }}</div>
                    <div class="text-caption font-weight-bold text-green-darken-1" v-if="slot.tipo === 'agendamento' && slot.preco">{{ (slot.preco || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }}</div>
                  </v-card-text>
                </v-card>
              </v-col>
            </v-row>
        ...
        <div v-if="$vuetify.display.mdAndUp && showChatPanel" class="chat-sidebar">
          <v-card flat height="100%" class="chat-container">
            ...
            <div class="typebot-wrapper">
              <TypebotChat :typebot-id="typebotId" :auto-open="true" :theme="chatTheme" @on-open="onChatOpen" />
            </div>
          </v-card>
        </div>
      </div>
...
.typebot-wrapper { height: calc(100vh - 280px); /* Ajuste aproximado da altura */ }
</style>
...
// Novas Funções para o Chat Integrado
